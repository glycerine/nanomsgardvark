...
SEXP nnSocket(SEXP domain_, SEXP protocol_) {
  ...
  if(TYPEOF(protocol_) != STRSXP) {
    REprintf("socket type (2nd arg, a.k.a. protocol) must be a string.\n");
    return R_NilValue;
  }

  int socket_type = string_to_socket_type(CHAR(STRING_ELT(protocol_,0)));
  if(socket_type < 0) {
    REprintf("socket type (2nd arg, a.k.a. protocol) not recgonized. Available socket types: {'NN_PAIR','NN_PUB','NN_SUB','NN_REP','NN_REQ','NN_PUSH','NN_PULL','NN_SURVEYOR','NN_RESPONDENT','NN_BUS'}.\n");
    return R_NilValue;
  }

  int domain_type = string_to_domain_type(CHAR(STRING_ELT(domain_,0)));
  if(domain_type < 0) {
    REprintf("domain type (1st arg) not found. Only 'AF_SP' and 'AF_SP_RAW' are available.\n");
    return R_NilValue;
  }
